...
export async function render(container) {
  ...
  const wrap = el('div');
  wrap.innerHTML = `<div class="dash-card"><h2>My public journalist profile</h2><p style="margin:8px 0 16px;color:var(--grey)">Readers can find this profile through newsroom search and the journalist directory.</p><form class="dash-form" id="journalist-profile-form">
  ...
  form.addEventListener('submit', async (event) => { event.preventDefault(); const button=form.querySelector('button'); const error=form.querySelector('.auth-error'); button.disabled=true; error.textContent=''; try { await api.patch('/auth/journalist-profile', Object.fromEntries(new FormData(form))); button.textContent='Saved'; setTimeout(()=>button.textContent='Save public profile',1500); } catch(err) { error.textContent=err instanceof ApiError?err.message:'Could not save profile.'; } finally { button.disabled=false; } });
